...
$datos = $conexion->query("SELECT * FROM compradetelas ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC);

// Etiquetas personalizadas
$json = "encabezados_compradetelas.json";
...
<h2 style="font-family:'Segoe UI', sans-serif; font-size:22px; color:#34495e;">📋 <?= strtoupper("compradetelas") ?></h2>

<!-- Botones de acción -->
<div style="margin-bottom:15px;">
    <a href="agregar_compradetelas.php" class="btn-accion">➕ Agregar</a>
    <a href="exportar_compradetelas.php" class="btn-accion btn-exportar">📥 Exportar Excel</a>
    <a href="importar_compradetelas.php" class="btn-accion btn-importar">📤 Importar Excel</a>
</div>
        ...
        <tbody>
            ...
            <tr>
                <?php foreach ($columnas as $col): ?>
                    <td>
                        <?php
                        // Detectar columnas monetarias y formatear con separador de miles y decimales
                        if (in_array($col['name'], ['costo_sin_iva', 'costo_con_iva', 'base', 'total_con_iva'])) {
                            $valor = str_replace(',', '.', $fila[$col['name']]); // Cambiar , por . para PHP
                            $numero = is_numeric($valor) ? (float)$valor : 0;
                            echo "$ " . number_format($numero, 2, ',', '.'); // Mostrar en formato colombiano
                        } else {
                ...
                <?php endforeach; ?>
                <td>
                    <a href="editar_compradetelas.php?id=<?= $fila['id'] ?>" class="btn-icon editar" title="Editar">✏️</a>
                    <a href="eliminar_compradetelas.php?id=<?= $fila['id'] ?>" onclick="return confirm('¿Eliminar este registro?')" class="btn-icon eliminar" title="Eliminar">🗑️</a>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
